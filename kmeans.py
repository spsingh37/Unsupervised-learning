...
def train_kmeans(train_data: np.ndarray, initial_centroids, *,
    ...
    for i in range(num_iterations):
        ...
        #######################################################################
        # ~~START DELETE~~
        distance = sklearn.metrics.pairwise_distances(centroids, train_data)  # [K x N]
        assignments = np.argmin(distance, axis=0)  # [N]
        errors = euclidean_distance(train_data, centroids[assignments])
        mean_error = errors.mean()

        # Update centroids
        # Note: it's possible to vectorize the following (not required).
...
def compress_image(image: np.ndarray, centroids: np.ndarray) -> np.ndarray:
    ...
    distance = sklearn.metrics.pairwise_distances(centroids, test_data)  # [K x N]
